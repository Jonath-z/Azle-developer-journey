...
function accountToIdentifier(account: Account): AccountIdentifier {
    ...
    const subaccountHex = account.subaccount ? Buffer.from(account.subaccount).toString('hex') : '';
    return `${account.owner.toText()}${subaccountHex}`;
}

// Example usage
const exampleAccountIdentifier: AccountIdentifier = accountToIdentifier(exampleAccount);
...
- [3 ckBTC and Bitcoin integration](/docs/developer-journey/level-3/bitcoin).