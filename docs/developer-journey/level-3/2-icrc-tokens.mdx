...
The most important difference between the ICP ledger and the ICRC-1 standard is their implementation of accounts. The ICP ledger uses `AccountIdentifier`s, which are essentially hashes of account values, providing a level of anonymity. The ICRC-1 ledger uses `Account`s, which are structured data types containing a principal and an optional subaccount.

ICRC-1 `Account` Definition
...
ICP `AccountIdentifier` Definition

```js
// ICP AccountIdentifier definition
type AccountIdentifier = string;

// Function to convert an ICRC-1 Account to an AccountIdentifier
function accountToIdentifier(account: Account): AccountIdentifier {
  // This is a simplified example. The actual conversion involves hashing.
  const subaccountHex = account.subaccount
    ? Buffer.from(account.subaccount).toString("hex")
    : "";
  return `${account.owner.toText()}${subaccountHex}`;
}

// Example usage
const exampleAccountIdentifier: AccountIdentifier =
  accountToIdentifier(exampleAccount);
console.log(exampleAccountIdentifier); // Outputs the hashed identifier
```

Key differences

- **Structure**: `Account` is a structure type with an owner and an optional subaccount, while `AccountIdentifier` is a hashed string representation.
- **Conversion**: An `Account` can be converted to an `AccountIdentifier`, but the reverse is not possible.
- **Usage**: `Account` provides more detailed account management, whereas `AccountIdentifier` offers a simpler, more anonymous representation.

for more details, you can refer to the [ICRC-1 repository](https://github.com/demergent-labs/ICRC-1).
...
The ICRC-1 token standard allows for several optional metadata fields to be specified. Metadata can be used to help improve the user experience and simplify the token's integration with wallets. Each metadata key is an arbitrary Unicode string that uses the format `<namespace>:<key>`, where the namespace `icrc1` is reserved for metadata defined for the ICRC-1 standard.

Common Metadata Fields

- `icrc1:symbol:` The token's currency code.

  **Example:** `variant { Text = "XTKN" }`

- `icrc1:name:` The token's name.

  **Example:** `variant { Text = "Test Token" }`

- `icrc1:decimals:` The number of decimals used by the token.

  **Example:** `variant { Nat = 8 }`

- `icrc1:fee:` The token's default transfer fee.

  **Example:** `variant { Nat = 10_000 }`
...
<Tabs>
  ...
  ```
  npx azle new icrc1_ledger_canister
  ```
...
Next, you'll use the `minter` identity you created in the last module, [4.1 Using the ICP ledger](/docs/developer-journey/level-3/ledger). If you need to export the `minter` identity's principal again, run the commands:
...
Then, set the identity that you want to use to deploy the ledger. In the previous module [1. Using the ICP ledger](/docs/developer-journey/level-3/1-ledger.mdx), you used the `DevJourney` identity, which you'll use again:
...
![Candid UI ICRC](../_attachments/icrc.png)
...
- [3. ckBTC and Bitcoin integration](/docs/developer-journey/level-3/3-bitcoin.mdx).
