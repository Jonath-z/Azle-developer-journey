...
keywords:
  [
    ...
    dfx deps,
    pullable canisters,
    third-party canisters,
  ]
---

# 3. Using third-party canisters
...
In this tutorial, you'll be focusing on pulling third-party canisters. If you're interested in developing third-party canisters that can be pulled by other developers, check out the [documentation on `dfx deps`](https://internetcomputer.org/docs/current/developer-docs/smart-contracts/maintain/import).
...
Use `npx azle new <project_name>` to create a new project and navigate into the project directory:
...
Then, create a new project called dependencies and nagivate into the new project directory:

```bash
npx azle new dependencies
cd dependencies
npm install
```

You'll use the default 'Hello world' project in this example, then you'll pull the Internet Identity canister as a dependency of our `backend` canister.

First, open the `dfx.json` file in your project's directory. You need to configure your `backend` canister to have the dependencies of `internet_identity`, with canister ID `rdmx6-jaaaa-aaaaa-aaadq-cai` on the mainnet. This is the canister ID of the DFINITY canister used to interact with ICP's identity and authentication product called Internet Identity. The developer journey will dive deeper into II in a later tutorial; for now, you'll just use it to demonstrate how third-party canisters can be used. You can do so by editing our `dfx.json` file to reflect the following configuration:
...
```json
{
  "canisters": {
    "rdmx6-jaaaa-aaaaa-aaadq-cai": {
      ...
      "wasm_hash": "83aec6c49dc5b7e4112ca2b45a0093da4e68fa88c69c4d2810fd917a241cda1f",
      "wasm_hash_download": "83aec6c49dc5b7e4112ca2b45a0093da4e68fa88c69c4d2810fd917a241cda1f",
...
The output will include a local Candid UI URL for both the `backend` canister, which has the default 'Hello world' code, and the local Candid UI URL for the Internet Identity canister.
...
- [4. Introduction to Candid](/docs/developer-journey/level-1/4-intro-candid.mdx).
